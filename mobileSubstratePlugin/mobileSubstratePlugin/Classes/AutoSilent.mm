...
static void ArrangeWakeUp(NSDate *time)
{
	if (time == nil)
		return;
	
	 Log(@"try arranging local notification");
	 UILocalNotification *localNotif = [[UILocalNotification alloc] init];
	 if (localNotif == nil)
	 return;
	localNotif.fireDate = time; //[[NSDate date] addTimeInterval:240];
	 localNotif.timeZone = [NSTimeZone defaultTimeZone];
	 
	 localNotif.alertBody = nil;
	 localNotif.alertAction = nil; // NSLocalizedString(@"View Vishal", nil);
	 
	 localNotif.soundName = nil;//UILocalNotificationDefaultSoundName;
	 ...
	 Log(@"done arranging local notification");
...
static void callback(CFNotificationCenterRef center, void *observer, CFStringRef name, const void *object, CFDictionaryRef userInfo) {
	...
	if(name != NULL)
	{
		...
		else if(CFStringCompare((CFStringRef)NotificationArrangeWakeUp, name, kCFCompareCaseInsensitive) == kCFCompareEqualTo)
		{
			[configManager RefreshAll];
			NSDate* wakeUpDate = [[[configManager GetNextWakeUpTime] retain] autorelease];
			//NSDate wakeupTime  = [NSDate [configManager nextWakeUpTime];
			ArrangeWakeUp(wakeUpDate);
		}		
...
void InitObserver()
{
	...
	CFNotificationCenterAddObserver(
									...
									NULL, // observer
									callback, // callback
									(CFStringRef)NotificationArrangeWakeUp, // name
									NULL, // object
									CFNotificationSuspensionBehaviorHold
									);
